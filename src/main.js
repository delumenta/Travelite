...
function modalView(){let m=state.modal;let heading={tripPicker:'Your trips',newTrip:'Create a trip',editTrip:'Edit your trip',deleteTrip:'Delete trip',stop:'Plan a stop',booking:'Booking details',expense:'Record an expense',catalog:'Add a discovery',profile:'Your account',confirm:'One more thing',password:'Choose a new password'}[m.type]||'Details';return `<div class="overlay" data-action="close-modal"><div class="modal ${m.type==='tripPicker'?'trip-modal':''}" ><div class="modal-head"><div><div class="eyebrow">TRAVELITE</div><h2>${heading}</h2></div><button class="icon-btn" data-action="close-modal" aria-label="Close">${icon('X')}</button></div>${modalContent(m)}</div></div>`;}
...
function chooseTrip(trip){state.trip=trip;state.search='';state.kind='all';state.foodFilter='all';state.foodTab='mine';state.foodCity='';state.foodArea='';state.foodCuisine='';state.foodSearch='';state.day=dayList(trip)[0]||today();state.assistant=[];state.threadId=null;localStorage.setItem(`travelite.trip.${state.user.id}`,String(trip.id));state.modal=null;state.mobileMenu=false;state.loading=true;render();loadTripData();if(trip.country)ensureCountryPhotos([trip.country]);}
...
document.addEventListener('submit',e=>{if(e.target.id==='assist-form'){e.preventDefault();askAssist(e.target);}else if(e.target.id==='google-search-form'){e.preventDefault();searchGoogle(e.target);}else if(['auth-form','trip-form','delete-trip-form','stop-form','booking-form','expense-form','catalog-form','password-form'].includes(e.target.id)){e.preventDefault();handleForm(e.target);}});
