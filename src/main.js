...
function foodCard(r,link){
  ...
  const ratingSummary=restaurantRatingSummary(r.id);
...
document.addEventListener('submit',e=>{if(e.target.id==='assist-form'){e.preventDefault();askAssist(e.target);}else if(e.target.id==='google-search-form'){e.preventDefault();searchGoogle(e.target);}else if(e.target.id==='add-search-form'){e.preventDefault();searchAddGoogle();}else if(['auth-form','trip-form','delete-trip-form','stop-form','booking-form','expense-form','catalog-form','password-form','rating-form'].includes(e.target.id)){e.preventDefault();handleForm(e.target);}});
