...
function openModal(type,data={}){if(type==='catalog'){state.googleResults=[];state.catalogSelection=null;state.googleBusy=false;}if(type==='addToDay'){state.addSearch='';state.addKind='all';state.addSelection=null;state.addNearby=false;state.googleResults=[];state.googleBusy=false;}if(type==='aroundStop'){state.aroundStop=data?.stop||null;state.aroundResults=[];state.aroundBusy=false;}state.modal={type,data};state.mobileMenu=false;render();if(type==='addToDay')setTimeout(()=>$('#add-day-search')?.focus(),0);}
